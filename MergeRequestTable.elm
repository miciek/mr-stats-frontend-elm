...
type Msg
    = UpdateListSuccess (List MergeRequestStats)
    ...
    case msg of
        UpdateListSuccess list ->
            let
                ...
                sortedList =
                    sortWith mrComparison list
            in
                ( { model | mrStats = sortedList }, Cmd.none )

        UpdateListFailed error ->
            ( { model | mrStats = [ { mergeRequest = { title = "ERROR: " ++ error, upvotes = 0, updated_at = "" }, timeToMerge = 0, commentsQty = 0, url = "" } ] }, Cmd.none )
...
init : ( Model, Cmd Msg )
init =
    ( { mrStats = [], lastRefreshTime = 0 }, mergeRequestsFetchEffect )
...
mergeRequestsFetchEffect =
    let
        request =
            Http.get backendUrl decodeMRs
    in
        Http.send updateList request


updateList : Result Http.Error (List MergeRequestStats) -> Msg
updateList result =
    case result of
        Result.Ok list ->
            UpdateListSuccess list
        ...
        Result.Err e ->
            UpdateListFailed (errorToString e)
...
errorToString e =
    case e of
        ...
        Http.BadStatus s ->
            "bad status - " ++ (toString s)

        Http.BadPayload c s ->
            "bad response - " ++ (toString c) ++ " - " ++ (toString s)
